...
const mockTournaments = [
  {
    id: "1",
    name: "EFA Brazil Cup 2026",
    slug: "efa-brazil-cup-2026",
    status: "in_progress",
  ...
  {
    id: "2",
    name: "Master League Europe",
    slug: "master-league-europe",
  ...
  {
    id: "3",
    name: "Lightning Tournament #47",
    slug: "lightning-tournament-47",
    status: "registrations_open",
    format: "knockout",
    game: "eFootball",
    teams: 6,
...
function HeroStats() {
  const stats = [
    ...
    { value: "EFA 500K", label: "In Prizes", icon: Award }
  ];
    ...
    >
      ...
      <div className="relative rounded-2xl overflow-hidden bg-surface-1/80 backdrop-blur-xl border border-white/10 hover:border-primary/50 transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-primary/20">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
          ...
          <Badge
            className={`${status.bgColor} ${status.color} border-0 gap-1.5 shadow-lg ${status.glowColor}`}
          >
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              {
                icon: Users,
                value: `${tournament.teams}/${tournament.maxTeams}`,
                label: "Teams",
                color: "primary",
              },
              {
              ...
              {
                icon: Calendar,
                value: new Date(tournament.startsAt).toLocaleDateString("en-US", {
                  day: "2-digit",
                  month: "short",
                }),
                label: "Start",
              ...
              {
                icon: Clock,
                value: new Date(tournament.endsAt).toLocaleDateString("en-US", {
                  day: "2-digit",
                  month: "short",
                }),
                label: "End",
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs text-muted-foreground">
              ...
              <span className="font-semibold text-foreground">
                {Math.round((tournament.teams / tournament.maxTeams) * 100)}%
              </span>
            </div>
          ...
          <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
            <Button
              className="w-full h-12 text-base font-semibold bg-gradient-to-r from-primary to-secondary hover:opacity-90 text-white shadow-lg shadow-primary/30"
              asChild
            >
              <Link to="/auth">
  ...
  return (
    ...
    >
      <div className="relative rounded-xl overflow-hidden bg-surface-1/60 backdrop-blur-sm border border-white/5 hover:border-primary/40 transition-all duration-300 hover:shadow-xl hover:shadow-primary/10">
        {/* Gradient Top Line */}
        <div
          className={`h-0.5 bg-gradient-to-r ${tournament.gradient} opacity-60 group-hover:opacity-100 transition-opacity`}
        />
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <Badge
              className={`${status.bgColor} ${status.color} border-0 gap-1 shrink-0`}
            >
              {tournament.status === "in_progress" && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 text-sm">
              ...
              <div className="flex items-center gap-1">{tournament.countryFlag}</div>
            </div>
            <Badge
              variant="secondary"
              className="bg-gradient-to-r from-secondary/20 to-primary/20 text-secondary font-semibold border-0"
            >
              {tournament.prize}
            </Badge>
          </div>
...
export default function PublicTournaments() {
  ...
  const filteredTournaments = mockTournaments.filter((tournament) => {
    ...
    if (activeTab === "upcoming")
      return (
        matchesSearch &&
        ["published", "registrations_open", "upcoming"].includes(tournament.status)
      );
    if (activeTab === "finished")
      return matchesSearch && tournament.status === "finished";
    return matchesSearch;
  });

  const featuredTournaments = mockTournaments.filter(
    (t) => t.featured && t.status !== "finished"
  );
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="container relative z-10 py-12 space-y-16">
        ...
        >
          ...
          >
            Join the biggest esports competitions and claim your glory
          </motion.p>
            ...
            >
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  The most anticipated competitions
                </p>
              </div>
            </motion.div>
        ...
        >
          ...
          <div className="relative text-center space-y-6">
            ...
            <p className="text-muted-foreground max-w-md mx-auto">
              Full tools to organize tournaments of any size
            </p>
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Button
                asChild
                size="lg"
                className="h-14 px-8 text-lg bg-gradient-to-r from-secondary to-primary hover:opacity-90 text-white shadow-xl shadow-secondary/30"
              >
                <Link to="/auth">
