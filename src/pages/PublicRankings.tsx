...
function PositionBadge({ position }: { position: number }) {
  ...
  if (position === 2) {
    return (
      <Medal className="h-7 w-7 text-slate-300 drop-shadow-[0_0_8px_rgba(203,213,225,0.5)]" />
    );
  }
  if (position === 3) {
    return (
      <Medal className="h-7 w-7 text-amber-500 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]" />
    );
  }
  return (
  ...
  return (
    <motion.div
      className={`relative group ${
        isFirst ? "md:scale-110 md:-translate-y-6 z-10" : ""
      }`}
    ...
    >
      ...
      {/* Card */}
      <div
        className={`relative bg-surface-1/80 backdrop-blur-xl rounded-2xl overflow-hidden border ${styles.border} transition-all duration-500 hover:shadow-2xl ${styles.glow}`}
      >
        {/* Animated top border */}
        <motion.div
        ...
        <div className={`${isFirst ? "p-8" : "p-6"} text-center relative`}>
          ...
          {/* Avatar */}
          <motion.div className="relative mx-auto mb-4" whileHover={{ scale: 1.1 }}>
            <div
              className={`absolute inset-0 rounded-full bg-gradient-to-br ${styles.bg} blur-md`}
            />
            <Avatar
              className={`${
                isFirst ? "h-24 w-24" : "h-20 w-20"
              } relative ring-4 ${styles.ring} ring-offset-2 ring-offset-background`}
            >
...
function RankingRow({
  ...
  return (
    ...
    >
      {/* Hover glow effect */}
      <div
        className={`absolute inset-0 bg-gradient-to-r ${styles.bg} opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl blur-sm`}
      />

      <div
        className={`relative grid grid-cols-12 gap-4 px-5 py-4 items-center rounded-xl transition-all duration-300 ${
          isTop3 ? "bg-surface-2/40" : "hover:bg-surface-2/30"
        } group-hover:border-l-2 ${styles.border}`}
      >
        {/* Position */}
        ...
        <div className="col-span-5 md:col-span-4 flex items-center gap-3">
          <motion.div className="relative" whileHover={{ scale: 1.1 }}>
            <Avatar
              className={`h-12 w-12 rounded-xl ring-2 ${
                isTop3 ? styles.ring : "ring-transparent"
              } group-hover:${styles.ring} transition-all`}
            >
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`font-semibold truncate ${styles.text} group-hover:text-primary transition-colors`}
              >
                {player.name}
              </span>
            </div>
...
function TeamCard({ team, index }: { team: typeof mockTeams[0]; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative bg-surface-1/70 backdrop-blur-xl rounded-2xl overflow-hidden border ${styles.border} transition-all duration-500 hover:shadow-2xl ${styles.glow}`}
      >
        {/* Top gradient */}
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              ...
              { value: team.titles, label: "Titles", color: "text-secondary" },
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <motion.div className="fixed inset-0 pointer-events-none" style={{ y: bgY }}>
        <img src={nebulaSoft} alt="" className="w-full h-[120%] object-cover opacity-40" />
      ...
      <div className="container relative z-10 py-12 space-y-12">
        {/* Hero Header */}
        <motion.div ref={heroRef} className="text-center space-y-6" style={{ opacity }}>
          <AnimatedSection>
            <motion.div
          ...
          <AnimatedSection delay={0.2}>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              Track the best players and teams worldwide competing in real time.
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-16 px-4">
              {[mockPlayers[1], mockPlayers[0], mockPlayers[2]].map((player, i) => (
                <div
                  key={player.id}
                  className={
                    i === 1 ? "md:order-2" : i === 0 ? "md:order-1" : "md:order-3"
                  }
                >
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-4 bg-surface-2/40 border-b border-border/30">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-foreground">Player Rankings</span>
                </div>
              ...
              <div className="grid grid-cols-12 gap-4 px-5 py-3 bg-surface-2/20 text-xs uppercase tracking-wider text-muted-foreground font-semibold border-b border-border/20">
                ...
                <div className="col-span-2 text-center hidden md:block">Matches</div>
        ...
        <AnimatedSection delay={0.6}>
          <motion.div className="text-center py-16 relative" whileHover={{ scale: 1.01 }}>
            {/* Background glow */}
            ...
            <div className="relative space-y-6">
              ...
              >
                ...
                <span className="text-sm font-semibold text-primary">
                  Want to show up on the leaderboard?
                </span>
              </motion.div>
              ...
              <p className="text-muted-foreground max-w-md mx-auto">
                Create your free account and enter the arena. Your results show up automatically on the global leaderboard.
              </p>
